...
#include <stdio.h>
#include <ros/ros.h> // Biblioteca principal do ROS (Robot Operating System)
#include <pthread.h> // Threads POSIX (controle de múltiplas execuções em paralelo)
#include <fcntl.h>   // Controle de arquivos (open, O_RDWR, etc.)
#include <errno.h>   // Códigos de erro do sistema (errno)
#include <termios.h> // Configuração de comunicação serial (baud rate, bits, paridade, etc.)
...
namespace ddsm115{
  ...
  enum DDSM115Command{
    ...
    COMMAND_GET_REALTIME_FEEDBACK = 0xA2,
